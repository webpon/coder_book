...
![image-20210811014124440](https://gitee.com/gitopenchina/typora-image/raw/master/img/20210811014124.png)
...
![image-20210811014055856](https://gitee.com/gitopenchina/typora-image/raw/master/img/20210811014055.png)
...
![image-20210811014320438](https://gitee.com/gitopenchina/typora-image/raw/master/img/20210811014320.png)
...
虽然上面也需要new Promise(),但我们使用ajax或者axios（下面会介绍ajax的实现原理）的时候，它们会自动new Promise的，不需要我们动手，这样就很方便了
...
```
const promise1 = Promise.resolve(3);const promise2 = 42;const promise3 = new Promise((resolve, reject) => {  setTimeout(resolve('foo'), 100);});Promise.all([promise1, promise2, promise3]).then((values) => {  console.log(values);   // [3, 42, "foo"]});
```
...
async 修饰的函数会返回一个Promise，return 的参数就是resolve（）括号里面的，我们可以使用then或者await接收它
...
<img src="https://gitee.com/gitopenchina/typora-image/raw/master/img/20210804005909.png" alt="image-20210804005908858" style="zoom:150%;" />



##### ajax实现原理

下面我们来看下Jquery中的ajax的简单实现
...
- 在main.js中直接import css文件
  ...
  ![image-20211001144805304](https://i.loli.net/2021/10/01/MrYoztds4via9lh.png)
...
- 直接使用<style>不加scoped属性
  ...
  ![image-20211001145201563](https://i.loli.net/2021/10/01/JkCW1cd48obE2pn.png)
...
![image-20211001200351031](https://i.loli.net/2021/10/01/ETNfjmbuSsB8Vp6.png)

![image-20211001200452735](https://i.loli.net/2021/10/01/JejbvApokOGLmwC.png)

## 生命周期

> Vue实例有一个完整的生命周期，也就是说从开始创建、初始化数据、编译模板、挂在DOM、渲染-更新-渲染、卸载等一系列过程，我们成为Vue 实例的生命周期，钩子就是在某个阶段给你一个做某些处理的机会。

![11370083-f279314aef6741db](C:\Users\web\Downloads\11370083-f279314aef6741db.webp)
...
![img](https://i.loli.net/2021/10/14/VYoTyuexA4w2QtP.png)

## Vue实例

> 我们在一个工程化的Vue项目中，通过webpack的打包，我们一般以main.js为入口，然后进行对导入的各种文件进行处理，最终合成汇总成js文件插入到html中

<img src="https://i.loli.net/2021/10/16/L23eb1HFhPd5xtQ.png" alt="image-20211016155330951" style="zoom: 67%;" />
...
<img src="https://i.loli.net/2021/10/16/EUVh4sMzouHetAb.png" alt="image-20211016170004005" style="zoom:67%;" />
...
![image-20211016172510954](https://i.loli.net/2021/10/16/jQnlA9zsC6DI2J8.png)
...
![image-20211016195603424](https://i.loli.net/2021/10/16/mtnIBYgcXlhWeOJ.png)

component(此时还没有生成真实DOM,然后这个步骤会把组件的所有子组件都生成组件实例的）:

![image-20211016195637635](https://i.loli.net/2021/10/16/qhNB6iaTjxnp5s3.png)

vm:

![image-20211016195801828](https://i.loli.net/2021/10/16/eHryp5VwZx9aOIb.png)
...
![image-20211016200250055](https://i.loli.net/2021/10/16/87WQvMHhrieLbNG.png)
...
![chrome-capture](https://i.loli.net/2021/10/12/8edOzvyFaKlYoSh.gif)
...
![Transition Diagram](https://i.loli.net/2021/10/12/jT4fzUmeMJdntZc.png)
...
![chrome-capture (1)](https://i.loli.net/2021/10/12/wVfDOypulQNS1vE.gif)
...
![chrome-capture (2)](https://i.loli.net/2021/10/12/gzWSsbm93TPD4Fn.gif)
...
![chrome-capture (3)](https://i.loli.net/2021/10/12/BgNlS6Qjec1qGit.gif)
...
![chrome-capture (4)](https://i.loli.net/2021/10/12/IUOf4CxFwqcLJVj.gif)
...
![image-20210925153951246](https://i.loli.net/2021/09/25/W5JqaoXZOMmsUE2.png)
...
上面的注意点输出:![image-20211018004837232](https://i.loli.net/2021/10/18/8CiMEfeLu9P1g6Z.png)
...
![img](https://i.loli.net/2021/10/07/bVGcQq6CRy53MOa.jpg)
...
作为Api调用工程师，我们大部分情况下只需要了解传输层和应用层即可，传输层能处理各种传输的数据，但是传输层的数据是无规则的，为了满足不同应用场景，我们需要多一个应用层，比如http协议

那具体流程是如何的呢？

应用层以规定好的格式编写数据，然后通过传输层传输，到了对应机器再翻译成相应的数据

![HTTP Request 请求报文格式](https://i.loli.net/2021/10/10/AzgsxurDbvZm5Yh.png)

<img src="https://i.loli.net/2021/10/07/6nLZq492wHzM1iv.png" alt="image-20211007200159069" style="zoom:67%;" />

假设我需要传输一个图片，那么对方接收到数据后是无法分辨是什么类型的数据的，那么这时我们就应该加上一下字段标注好对应的数据信息一起传过去，另一边也懂该协议，那么就可以正常解析数据了，这就是应用层的意义。

下面我们会模拟一下http协议的实现，不过不是现在
...
![image-20211007210553571](https://i.loli.net/2021/10/07/XjWxLa1yEbcoB2Q.png)


三次握手过程理解![img](https://i.loli.net/2021/10/07/yxR4UphuOGB3Ciz.png)
...
`如果没有三次握手，由于客户端发起请求时，请求的syn包有可能长时间都没有发送过去，这时候客户端就会废弃掉这个资源，后面服务器再收到syn包，如果没有进行后两次的确认就直接建立连接，则建立了一个无效连接，浪费服务器资源`

四次挥手过程理解 

![img](https://i.loli.net/2021/10/07/fqEbAJ8gPeowlQM.png)

1）客户端进程发出连接释放报文，并且停止发送数据。释放数据报文首部，FIN=1，此时，客户端进入FIN-WAIT-1（终止等待1）状态。。
      2）服务器收到连接释放报文，发出确认报文，ACK=1，ack=u+1，并且带上自己的序列号seq=v，此时，服务端就进入了CLOSE-WAIT（关闭等待）状态。TCP服务器通知高层的应用进程，客户端向服务器的方向就释放了，这时候处于半关闭状态，即客户端已经没有数据要发送了，但是服务器若发送数          			据，客户端依然要接受。这个状态还要持续一段时间，也就是整个CLOSE-WAIT状态持续的时间。
...
> command.js是完整的 [node.js](http://nodejs.org/) 命令行解决方案。

## cmd命令行

### 简介

> 在cmd.exe或者powershell.exe程序下可执行的一些指令, 其中包括一直内置指令，比如cd、dir，这种是一些功能指令
>
> 命令行还可以直接打开程序，比如输入calc、ping就能打开对应的程序
>
> 但是这得在系统环境变量配置，不然就得进入程序目录下才行

我们平时使用node index.js就是执行node.js程序，因为我们已经配置了node.js的环境变量，所以能在全局使用node命令，后面的index.js则是参数

![image-20211017003219492](https://i.loli.net/2021/10/17/9ZL5enIlUfHwixW.png)

<img src="https://i.loli.net/2021/10/17/Xa3dT4FuyGBJptC.png" alt="image-20211017003251256" style="zoom: 67%;" />

为什么我们能直接使用npm、npx这些命令呢？这些就是功能指令了，也是一种程序，只不过以.cmd为后缀，我们输入npm实际上就是让cmd.exe执行npm.cmd其中的命令

因为我们的npm.cmd和npx.cmd就在nodejs的目录中，在系统环境变量注册了，所以能在任何地方运行

<img src="https://i.loli.net/2021/10/17/zOKqQBr74ZuMVwm.png" alt="image-20211017003647522" style="zoom: 67%;" />
...
那么我们想创建一个新的命令咋办，我们可以手写一个test.cmd，然后编写命令，再使用cmd.exe执行它即可，但是这得会一点.cmd的脚本命令编写能力

当然，作为前端开发来说，这不是我们所需要的，我们仅仅想自定义一个命令并且能执行对应js的指令

这时我们可以借助npm提供的api，npm link生成

**第一步：**

​		现在对应目录下执行npm init

​		<img src="https://i.loli.net/2021/10/17/5g9eVW18SQOJKP6.png" alt="image-20211017004237570" style="zoom:67%;" />

​		这时就会生成一个package.json了

**第二步：**

​		新建一个js文件，并且首行必须为#!/usr/bin/env node开头

​		index.js:		

```
     #!/usr/bin/env node
     console.log('测试自定义命令行')			
```

**第三步：**

​		在package.json中加上bin字段
...
![image-20211017005150783](https://i.loli.net/2021/10/17/nvN1qAZlhQUi47c.png)

​		这条命令会帮我们生成.cmd程序，程序中的命令，则是使用node.exe打开index.js这个功能

​		我们可以输入npm root -g查看生成的.cmd程序存放在哪里

![image-20211017005552703](https://i.loli.net/2021/10/17/Hxq8LPQoeYCKJNu.png)

这时我们可以看到对应的.cmd了

<img src="https://i.loli.net/2021/10/17/FcE92XRweNlmhSs.png" alt="image-20211017005631346" style="zoom:67%;" />

当然这个文件夹也是在环境变量注册了的，所以我们可以在任何地方执行它

<img src="https://i.loli.net/2021/10/17/pf6atjONEl2ACok.png" alt="image-20211017005845887" style="zoom:67%;" />

**测试**

![image-20211017005951883](https://i.loli.net/2021/10/17/eE1sWMQKu8ibzXq.png)

成功执行了我们想执行的inde.js

`以上步骤需要使用npm link参与生成.cmd程序，但是当我们把npm包发布，再通过-g全局安装则可以免除npm link操作，-g会自动执行npm link这个命令的`

*附：*

![image-20211017010525044](https://i.loli.net/2021/10/17/zV7Eyos34c5eYpj.png)
